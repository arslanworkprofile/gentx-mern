  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link to="/admin/products/new" className="btn-primary flex items-center gap-1 text-xs md:text-sm px-3 py-2 md:px-4 md:py-2.5">
      ...
      {loading ? <PageSpinner /> : (
        <>
          ...
          <div className="md:hidden space-y-3">
            {products.map(p => (
              <div key={p._id} className="admin-card !p-3">
                <div className="flex gap-3">
                  <img src={p.images?.[0]?.url || 'https://images.unsplash.com/photo-1441984904996-e0b6ba687e04?w=60'} alt={p.name} className="w-14 h-16 object-cover bg-gray-100 flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm truncate">{p.name}</p>
                    <p className="text-xs text-gray-400 capitalize">{p.category} · {p.brand}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-sm font-semibold">${(p.discountPrice > 0 ? p.discountPrice : p.price).toFixed(2)}</p>
                      <span className={`text-[10px] font-medium ${p.stock === 0 ? 'text-red-500' : p.stock < 10 ? 'text-yellow-600' : 'text-green-600'}`}>
                ...
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-50">
                  <button onClick={() => handleToggle(p._id)}
                    className={`text-[10px] tracking-widest uppercase font-medium px-2.5 py-1 border flex-1 text-center ${p.isActive ? 'border-green-200 text-green-600' : 'border-red-200 text-red-500'}`}>
                    {p.isActive ? 'Active' : 'Hidden'}
                  </button>
                  <Link to={`/admin/products/edit/${p._id}`}
                    className="text-[10px] tracking-widest uppercase border border-gray-200 px-3 py-1 flex-1 text-center">
                    Edit
                  </Link>
          ...
          <div className="admin-card overflow-x-auto hidden md:block">
                  ...
                  <tr key={p._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="table-cell text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link to={`/admin/products/edit/${p._id}`}
                          className="text-xs tracking-widest uppercase border border-gray-200 px-3 py-1.5 hover:border-black transition-colors">Edit</Link>
                        <button onClick={() => handleDelete(p._id, p.name)}
                          className="text-xs tracking-widest uppercase border border-red-100 text-red-400 px-3 py-1.5 hover:bg-red-500 hover:text-white hover:border-red-500 transition-colors">Del</button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {products.length === 0 && <div className="text-center py-16 text-gray-400">No products found</div>}
            <div className="mt-4"><Pagination page={page} pages={pages} onPageChange={setCurrentPage} /></div>
          </div>
          ...
          {products.length === 0 && <div className="text-center py-16 text-gray-400 md:hidden">No products found</div>}
        </>
      )}
    </div>
  );
